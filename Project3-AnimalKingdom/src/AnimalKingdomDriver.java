...
public class AnimalKingdomDriver {
	...
	public static void main(String[] args) {
		...
		for(Animal animal : animalList) {
			if(animal instanceof WaterDweller) {
				...
				numWaterDwellersBreatheAir +=
						((WaterDweller) animal).breathesAir() ? 1 : 0;
				numWaterDwellersBreatheWater = numWaterDwellers -
						numWaterDwellersBreatheAir;
			}
		}
		...
		for(Animal animal : animalList) {
			if(animal instanceof WaterDweller) {
				System.out.println(animal.getName() + " breathes "
						+ (((WaterDweller) animal).breathesAir() ?
								"air" : "water"));
			}
		}
		...
		// YOUR PART C CODE HERE
		for(Animal animal : animalList) {
			numEndangered += animal instanceof Fish
					&& ((Fish) animal).isEndangered() ? 1 : 0;
			numEndangered += animal instanceof Mammal
					&& ((Mammal) animal).isEndangered() ? 1 : 0;
			numEndangered += animal instanceof Bird
					&& ((Bird) animal).isEndangered() ? 1 : 0;
		}
		...
		for(Animal animal : animalList) {
			...
			if(animal instanceof Fish && ((Fish) animal).isEndangered()) {
				isEndangered = true;
			}
			else if(animal instanceof Mammal
					&& ((Mammal) animal).isEndangered()) {
				isEndangered = true;
			}
			else if(animal instanceof Bird && ((Bird) animal).isEndangered()) {
				isEndangered = true;
			}
			if(isEndangered) {
				System.out.print(animal.getName() + ": ");
		...
		Kakapo kakapo = new Kakapo(99, "Kevin the Kākāpō");
		System.out.println("\n\nBONUS ANIMAL:\n" + kakapo);
		System.out.print(kakapo.getName() + " conservation information: ");
		((Endangered) kakapo).displayConservationInformation();
